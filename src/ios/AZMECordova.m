...
#define AZME_PLUGIN_VERSION @"2.3.0"
...
- (void)didReceiveDataPush:(NSString*)_category withBody:(NSString*)_body
{
    NSString* jsString = [NSString stringWithFormat:@"AzureEngagement.handleDataPush({\"category\":\"%@\",\"body\":\"%@\"});", _category,_body ];
...
{
    NSString* jsString = [NSString stringWithFormat:@"AzureEngagement.handleOpenURL(\"%@\");", _url];
    [self.commandDelegate evalJs:jsString];
...
- (void)checkRedirect:(CDVInvokedUrlCommand*)command
{
    ...
    CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK  messageAsString:nil ];
     ...
     else if ([type compare:@"data"]==NSOrderedSame)
     {
         [engagementSharedSingleton enablePush];
     }
     else{
      pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR
                                       messageAsString:@"unsupported checkRedirect type" ];
     }
    [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
}
